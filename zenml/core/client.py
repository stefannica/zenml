...
class Client:
    """ZenML client definition. This is a Singleton class.
    ...
    def __init__(self):
        ...
        if Client.__instance__ is None:
            try:
                ...
                self.path = Client.get_zenml_dir(os.getcwd())
            except Exception:
                # If there isnt a zenml.config, use the cwd
                self.path = os.getcwd()

            if not path_utils.is_dir(self.path):
                raise Exception(f'{self.path} does not exist or is not a dir!')
            ...
            Client.__instance__ = self
        else:
            raise AssertionError(
                "You cannot create another Service class, its a Singleton!")
